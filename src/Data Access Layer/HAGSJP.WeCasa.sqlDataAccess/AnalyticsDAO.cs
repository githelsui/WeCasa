...
namespace HAGSJP.WeCasa.sqlDataAccess
{
	public class AnalyticsDAO
	{
        ...
        public DAOResult GetRegistrationsPerDay(DateTime minDate)
        {
            var result = new DAOResult();
            var loginsPerDate = new List<Dictionary<String, String>>();
            _connectionString = BuildConnectionString().ConnectionString;
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                var command = connection.CreateCommand();
                command.CommandText = selectSql;
                command.Parameters.AddWithValue("@operation", "Registration");
                command.Parameters.AddWithValue("@min_date", minDate);
                command.Parameters.AddWithValue("@message", "Account created successfully");
            ...
            result.IsSuccessful = false;
            result.Message = "Failed fetching registrations per day";
            return result;
        }
    }
}
