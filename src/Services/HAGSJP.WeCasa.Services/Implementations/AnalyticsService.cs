...
namespace HAGSJP.WeCasa.Services.Implementations
{
	public class AnalyticsService
	{
        ...
        public KPIResult GetRegistrationsPerDay(UserAccount ua, DateTime minDate)
        {
            try
            {
                var result = new KPIResult();

                var daoResult = _dao.GetRegistrationsPerDay(minDate);
                if (!daoResult.IsSuccessful)
                {
                    _logger.Log("Get registrations per day failed", LogLevels.Error, "Data Store", ua.Username);
                    result.IsSuccessful = false;
                    result.Message = daoResult.Message;
                }
                else
                {
                    _logger.Log("Get registrations per day success", LogLevels.Info, "Data Store", ua.Username);
                    var regPerDay = (List<Dictionary<String, String>>)daoResult.ReturnedObject;
                    var minDateStr = minDate.ToShortDateString();
                    var maxDateStr = DateTime.Now.Date.ToShortDateString();
                    var addMinDate = true;
                    var addMaxDate = true;
                    foreach (Dictionary<String, String> loginData in regPerDay)
                    {
                        var date = loginData["date"];
                        if (date == minDateStr)
                        {
                    ...
                    if (addMinDate)
                    {
                        ...
                        regPerDay.Insert(0, minDateValue);
                    }
                    ...
                    if (addMaxDate)
                    {
                        ...
                        regPerDay.Add(maxDateValue);
                    }

                    result.ReturnedObject = regPerDay;
                    result.IsSuccessful = true;
                    result.Message = daoResult.Message;
                }
                return result;
